...
def Camera_set_position(cam, x, y, z):
...
scene = Scene3D({ 'handle': 0 })
camera = Camera3D({ 'handle': 0 })
renderer = Renderer3D({ 'handle': 0 })
sun = Mesh3D({ 'handle': 0 })
earth = Mesh3D({ 'handle': 0 })
moon = Mesh3D({ 'handle': 0 })
time = 0
def animate():
    global time 
    time = time + 0.01
...
def init():
    global scene, camera, renderer, sun, earth, moon 
    scene = Scene_create()
