...
result_label = Label({ 'handle': 0, 'id': "", 'text': "" })
def on_greet():
...
def create_app():
    ...
    config = WindowConfig({ 'title': "Omni GUI", 'width': 400, 'height': 300, 'background': "#f5f5f5" })
    win = Window_create(config)
    Label_create(win, "Welcome to Omni GUI", 20, 20);
    name_input = TextInput_create(win, 20, 70);
    btn = Button_create(win, "Greet", 20, 110)
...
__all__ = ["on_greet", "create_app", "main"]
